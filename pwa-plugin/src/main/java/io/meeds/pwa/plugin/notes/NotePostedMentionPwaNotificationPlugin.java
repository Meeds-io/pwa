...
@Profile("notes")
@Component
public class NotePostedMentionPwaNotificationPlugin implements PwaNotificationPlugin {
  ...
  public PwaNotificationMessage process(NotificationInfo notification, LocaleConfig localeConfig) {
    ...
    notificationMessage.setTitle(resourceBundleService.getSharedString(TITLE_LABEL_KEY, localeConfig.getLocale())
                                                      .replace("{0}", notification.getValueOwnerParameter("CURRENT_USER")));
    notificationMessage.setBody(notification.getValueOwnerParameter("NOTE_TITLE"));
//    notificationMessage.setUrl(permanentLinkService.getPermanentLink(new PermanentLinkObject("notes",
