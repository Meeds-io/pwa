...
@Profile("task")
@Component
public class TaskMentionedPwaNotificationPlugin implements PwaNotificationPlugin {
  private static final String   TITLE_LABEL_KEY = "pwa.notification.TaskMentionedPlugin.title";
  ...
  public PwaNotificationMessage process(NotificationInfo notification, LocaleConfig localeConfig) {
    ...
    notificationMessage.setTitle(resourceBundleService.getSharedString(TITLE_LABEL_KEY, localeConfig.getLocale())
                                                      .replace("{0}",
                                                               getFullName(notification.getValueOwnerParameter("creator"))));
