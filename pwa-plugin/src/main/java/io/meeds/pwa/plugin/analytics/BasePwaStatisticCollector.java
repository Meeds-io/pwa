...
public abstract class BasePwaStatisticCollector {
  ...
  protected StatisticData addSubscriptionDetails(UserPushSubscription subscription, String username) {
    ...
    statisticData.setSubModule("installation");
    statisticData.setUserId(Long.parseLong(getIdentityId(username)));
    statisticData.addParameter("deviceId", subscription.getDeviceType());
    statisticData.addParameter("subscriptionId", subscription.getId());
    statisticData.addParameter("subscriptionSite", getSubscriptionDomain(subscription.getEndpoint()));
